    ...
    for k, v in src_model['model'].items():
        ...
        if 'backbone.fpn_lateral' in k:
            ...
            if key_name_split[1] == 'box_head':
                fc_id = int(key_name_split[3][-1]) - 1
                name = f'roi_head.bbox_head.{int(key_name_split[2])}.shared_fcs.{fc_id}.{key_name_split[-1]}'
            ...
            elif 'roi_heads.box_predictor' in k:
                if 'cls_score' in k:
                    name = k.replace('box_predictor', 'bbox_head').replace('cls_score', 'fc_cls')
                    name = name.replace('roi_heads', 'roi_head')
                elif 'bbox_pred' in k:
                    name = k.replace('box_predictor', 'bbox_head').replace('bbox_pred', 'fc_reg')
                    name = name.replace('roi_heads', 'roi_head')
        ...
        if not isinstance(v, torch.Tensor):
            if 'roi_head.mask_head' in name:
                # weight share
                for i in range(3):
                    name1 = name.replace('roi_head.mask_head', f'roi_head.mask_head.{i}')
                    dst_state_dict[name1] = torch.from_numpy(v)
            else:
...
# d2 cascade mask rcnn 配置 configs/Misc/cascade_mask_rcnn_R_50_FPN_1x.yaml
# model_final_e9d89b.pkl 42.1/36.4
def main():
...
# /home/PJLAB/huanghaian/Downloads/model_final_e9d89b.pkl cascade.pth
