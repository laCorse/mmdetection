    ...
    for k, v in src_model['model'].items():
        ...
        if 'backbone.fpn_lateral' in k:
            ...
            if key_name_split[1] == 'cls_subnet':
                if int(key_name_split[2]) not in [1, 4, 7, 10]:
                    fc_id = int(key_name_split[2]) // 3
                    name = f'bbox_head.cls_convs.{fc_id}.conv.{key_name_split[-1]}'
                else:
                    fc_id = (int(key_name_split[2])-1)//3
                    name = f'bbox_head.cls_convs.{fc_id}.gn.{key_name_split[-1]}'
                ...
                if int(key_name_split[2]) not in [1, 4, 7, 10]:
                    ...
                    fc_id = (int(key_name_split[2])-1)//3
                    name = f'bbox_head.reg_convs.{fc_id}.gn.{key_name_split[-1]}'
            ...
            elif key_name_split[1] == 'ctrness':
                name = f'bbox_head.conv_centerness.{key_name_split[-1]}'
            else:
                print(f'{k} is invalid')
...
# d2 fcos 配置 configs/COCO-Detection/fcos_R_50_FPN_1x.py
# 权重是自己训练, mAP 39.3
def main():
